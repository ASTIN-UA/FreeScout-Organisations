...
**Integrações opcionais:** [End-User Portal](https://freescout.net/module/end-user-portal/), [API and Webhooks](https://freescout.net/module/api-webhooks/), [Kanban](https://freescout.net/module/kanban/), [Custom Fields](https://freescout.net/module/custom-fields/)

> [!IMPORTANT]
> **Instale sempre a partir da [versão mais recente](https://github.com/ASTIN-UA/FreeScout-Organisations/releases/latest), não a partir do código-fonte do repositório.**
> Transfira `OrgPortal.zip` da página de Releases — contém a estrutura de diretórios correta exigida pelo FreeScout.
> Transferir o código-fonte (via "Code → Download ZIP" ou `git clone`) **não funcionará** e irá corromper a estrutura do módulo.
> As atualizações automáticas também requerem que o ZIP da versão tenha sido utilizado para a instalação inicial.
...
- [O que o OrgPortal acrescenta ao FreeScout](#o-que-o-orgportal-acrescenta-ao-freescout)
...
- [Domínios de E-mail — Adesão Automática](#domínios-de-e-mail--adesão-automática)
...
- [Sino de Notificação em Tempo Real](#sino-de-notificação-em-tempo-real-opcional)
- [Subscrições de Notificação](#subscrições-de-notificação-opcional)
- [Definições de Organização do Portal](#definições-de-organização-do-portal)
- [Modelos de E-mail de Notificação Multilingues](#modelos-de-e-mail-de-notificação-multilingues-opcional)
- [REST API](#rest-api-opcional)
- [Instalação](#instalação)
- [Atualizações Automáticas](#atualizações-automáticas)
- [Compatibilidade de Módulos](#compatibilidade-de-módulos)
...
- **Adesão automática por domínio de e-mail** — associe `company.com` a uma organização e cada cliente que escreve a partir dele é automaticamente adicionado e atribuído
...
![Lista de organizações — filtro de estado, pesquisa em tempo real, emblemas coloridos, etiquetas, contagens de pedidos](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/org-list.png)
...
![Edição de organização — seletor de cores, chips de etiquetas, tabela de membros com funções e unidades](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/org-edit.png)
...
![Edição de cliente — campo de organização com sugestões automáticas e seletor de função](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/customer-org-field.png)
...
![Edição de organização — membros com funções e unidades, painel de gestão de unidades](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/org-edit.png)

---

## Domínios de E-mail — Adesão Automática

*Pare de adicionar os mesmos funcionários da empresa um a um.*

Associe um domínio de e-mail a uma organização e cada cliente que escreve a partir desse domínio é automaticamente atribuído a ela e adicionado como membro — sem passos manuais, nada para esquecer quando uma nova pessoa envia um e-mail pela primeira vez.

Configurado por organização em **Gerir → Organizações → Editar → Domínios de e-mail**.

### Como funciona a correspondência

| Regra | Comportamento |
|-------|----------|
| **Apenas correspondência exata** | `company.com` corresponde a `jane@company.com`. **Não** corresponde a `jane@mail.company.com` ou `jane@www.company.com` — adicione-os como entradas separadas se desejado |
| **Normalização** | `@Company.COM`, `https://www.company.com/` e `company.com.` são todos guardados como `company.com` |
| **A atribuição manual tem sempre precedência** | Um cliente que já pertence a outra organização nunca é movido. Contratantes e decisões deliberadas do administrador estão seguros |
| **A revogação é permanente** | Desativar um membro é permanente até que uma pessoa o reverta. O cliente pode continuar a enviar e-mails; a automatização não restaurará o acesso |
| **Âmbito de caixa de correio** | Um domínio numa organização específica de caixa de correio só se aplica dentro dessa caixa de correio. Uma associação específica de caixa de correio tem precedência sobre uma global para o mesmo domínio |
| **Múltiplos domínios** | Uma organização pode ter quantos domínios precisar (`company.com`, `company.co.uk`, uma marca adquirida…) |

### Fornecedores públicos são bloqueados

`gmail.com`, `outlook.com`, `ukr.net`, `icloud.com`, serviços de e-mail descartável e similares são **rejeitados ao guardar**. Associar um colocaria centenas de clientes não relacionados numa única organização e — através do Portal de Utilizadores Finais — lhes daria acesso aos pedidos mútuos.

A lista é fornecida com o módulo e pode ser **estendida** (nunca reduzida) através da opção `orgportal.public_domains_extra` para fornecedores regionais. Um fallback codificado garante que os principais fornecedores permaneçam bloqueados mesmo se o ficheiro de configuração estiver em falta ou corrompido.

As organizações desativadas param de adicionar clientes completamente.

### Clientes existentes

Uma associação só afeta e-mails futuros: clientes que já existem não são adicionados retroativamente. São processados assim que escrevem novamente.

### Remover uma associação

Remover um domínio interrompe a atribuição automática futura. Os membros que já criou são **retidos por predefinição** — podem já estar a utilizar o portal. É-lhe perguntado em separado se deseja desativá-los; este rollback toca apenas membros adicionados por esse domínio específico, nunca os adicionados manualmente.

Os membros adicionados automaticamente são marcados com um emblema **@** na lista de membros.
...
**Os domínios de e-mail servem como último recurso** nos modos `member` e `tag`: quando nem uma associação de etiqueta nem uma adesão existente resolve o pedido, o domínio de e-mail do autor é verificado. Nunca substitui nenhum deles, portanto uma regra de etiqueta ou uma atribuição manual do administrador tem sempre precedência. No modo `tag_only`, a correspondência de domínio não é utilizada.
...
![Separador Sistema — fonte de atribuição, barra de progresso, estatísticas preliminares, controlos de preenchimento](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/attribution-settings.png)
...
![Permissões granulares — permitir gerir organizações e modelos de notificação](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/user-permissions.png)
...
![Separador Sistema — painel do seletor de idioma do portal com caixas de verificação de idioma](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/system-settings.png)
...
![Pedidos da Empresa — tabela completa com indicadores de leitura, banner de filtro por autor, filtros de estado](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-tickets.png)
...
![Vista de pedido no portal — formulário de resposta com anexos por arrastar e largar e banner de pedido fechado](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-reply.png)
...
![Acompanhamento de visualização pelo gestor — nota 'visualizado' aparece sob a resposta do agente na vista de pedido de administrador](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/manager-viewed.png)
...
![Sino de notificação em tempo real — painel suspenso com notificações não lidas agrupadas](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-bell.png)
...
![Matriz de subscrição de notificação — alternâncias por unidade e por membro](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-subscriptions.png)
...
![Definições de Organização do Portal — separadores Unidades e Membros](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-settings.png)
...
![Modelos de e-mail — painéis expansíveis por idioma, botão Carregar Predefinição, editor Summernote](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/admin-templates.png)
...
> [!IMPORTANT]
> Transfira `OrgPortal.zip` da **[página de Releases](https://github.com/ASTIN-UA/FreeScout-Organisations/releases/latest)** — **não** utilize "Code → Download ZIP" nem clone o repositório. Apenas o ZIP da versão tem a estrutura correta para o FreeScout e suporta atualizações automáticas.

1. Transfira `OrgPortal.zip` da [versão mais recente](https://github.com/ASTIN-UA/FreeScout-Organisations/releases/latest)
2. Extraia e copie a pasta `OrgPortal` para `Modules/` da sua instalação FreeScout
...
![Definições por caixa de correio — visibilidade do emblema e filtros de estado do Kanban com etiquetas multilingues](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/mailbox-settings.png)
...
| ![Separador Sistema](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/system-settings.png) | ![Edição de cliente](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/customer-org-field.png) |
...
| ![Portal de Pedidos da Empresa](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-tickets.png) | ![Resposta no portal](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-reply.png) |
| *Pedidos da Empresa — tabela, filtro por autor, indicadores de leitura* | *Pedido no portal — resposta com anexos, banner de fechado* |
| ![Definições de Organização do Portal](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-settings.png) | ![Sino de notificação](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-bell.png) |
| *Definições de Organização do Portal — separadores Unidades e Membros* | *Sino de notificação em tempo real com painel suspenso* |
| ![Matriz de subscrição](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/portal-subscriptions.png) | ![Modelos de e-mail](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/admin-templates.png) |
...
| ![Integração com o Kanban](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/kanban-org.png) | ![Definições por caixa de correio](https://raw.githubusercontent.com/ASTIN-UA/FreeScout-Organisations/main/docs/screenshots/mailbox-settings.png) |
