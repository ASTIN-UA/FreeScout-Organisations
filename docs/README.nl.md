...
| `GET` | `/api/organizations/{id}` | Organisatie met leden ophalen |
| `PUT` | `/api/organizations/{id}` | Organisatie bijwerken |
| `DELETE` | `/api/organizations/{id}` | Organisatie verwijderen |
| `GET` | `/api/customers/{id}/organization` | Klantorganisatie |
| `PUT` | `/api/customers/{id}/organization` | Lidmaatschap instellen/bijwerken |
| `DELETE` | `/api/customers/{id}/organization` | Verwijderen uit organisatie |
...
Wijst een klant toe aan een organisatie of werkt zijn rol bij. **Één klant — één organisatie**: als de klant al lid is van *een andere* organisatie, wordt het verzoek afgewezen met `409 Conflict`. Om over te dragen — verwijder eerst het huidige lidmaatschap via `DELETE`.
...
**200 OK** *(rol bijgewerkt of geen-operatie)*
```json
{"success": true, "message": "Membership updated."}
```

**409 Conflict** *(klant al in andere organisatie)*
```json
{
