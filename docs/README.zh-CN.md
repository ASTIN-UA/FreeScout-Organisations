...
- **通过电子邮件域自动成员关系** — 将 `company.com` 绑定到组织，写入该域的每个客户都会自动注册并被分配
...
将电子邮件域绑定到组织，从该域写入的每个客户都会被自动归属和注册为成员——无需手动步骤，当新员工第一次发送邮件时无需担心遗漏。

在**管理 → 组织 → 编辑 → 电子邮件域**中按组织配置。

### 匹配如何工作

| 规则 | 行为 |
|------|------|
| **仅精确匹配** | `company.com` 匹配 `jane@company.com`。它**不**匹配 `jane@mail.company.com` 或 `jane@www.company.com` — 如需匹配请作为单独条目添加 |
| **规范化** | `@Company.COM`、`https://www.company.com/` 和 `company.com.` 都存储为 `company.com` |
| **手动分配始终优先** | 已属于其他组织的客户从不被移动。承包商和管理员的有意决定是安全的 |
| **撤销保留** | 停用成员是永久的，直到人工撤销。客户可继续发送邮件；自动化不会恢复访问权限 |
| **邮箱范围** | 邮箱特定组织上的域仅适用于该邮箱。邮箱特定绑定优先于全局绑定 |
| **多个域** | 组织可拥有任意数量的域（`company.com`、`company.co.uk`、收购的品牌…） |

### 公共提供商被阻止

`gmail.com`、`outlook.com`、`ukr.net`、`icloud.com`、一次性邮件服务等类似服务在**保存时被拒绝**。绑定一个会将数百个无关客户吸入单个组织，并——通过 End-User Portal——让他们能访问彼此的工单。

列表随模块提供，可通过 `orgportal.public_domains_extra` 选项**扩展**（从不缩减）用于区域性提供商。硬编码的回退确保即使配置文件缺失或损坏，主要提供商仍保持被阻止。

停用的组织完全停止客户注册。

### 添加现有客户

新绑定仅影响未来的邮件。要注册已存在的客户，新绑定仅影响未来的邮件：已存在的客户不会被追溯注册。他们在再次发送邮件时会被采集。

### 删除绑定

删除域停止未来自动分配。它已创建的成员**默认保留** — 他们可能已在使用门户。您会被单独询问是否停用他们；该回滚仅触及由该特定域注册的成员，从不触及手工添加的成员。
...
**电子邮件域在 `member` 和 `tag` 模式中充当最后回退** — 当标签绑定和现有成员关系都无法解决工单时，会检查作者的电子邮件域。它从不覆盖要么，所以标签规则或管理员的手动分配始终优先。在 `tag_only` 模式下不使用域匹配。
...
*在门户工单页面直接展示自定义字段模块的数据。*

需要安装并启用 [Custom Fields](https://freescout.net/module/custom-fields/) 模块。

- 在"邮箱设置 → OrgPortal"中，每个邮箱都有一个面板，可让您选择哪些自定义字段显示在门户工单页面上
- 拖动字段以重新排序；每个字段可以为每种门户语言设置自定义标签，若未设置则回退到已保存的英文标签，再回退到字段原始名称
- 在门户工单页面上，已启用的字段会在工单主题和消息串之间以响应式两栏网格显示——仅显示有非空值的字段
- 完全可选——当未安装或未启用自定义字段模块时，面板和工单页面上的模块会自动隐藏
