...
OrgPortal poskytuje kompletné REST API na správu organizácií, štrukturálnych jednotiek a členstiev zákazníkov — autentifikácia pomocou hlavičky `X-FreeScout-API-Key` alebo parametra dotazu `api_key`.

📖 **Úplná referencia API → [docs/api/README.sk.md](api/README.sk.md)** (všetky koncové body, príklady požiadaviek/odpovedí, chybové kódy)

Interaktívna dokumentácia ReDoc je dostupná aj v **Manage → API & Webhooks → OrgPortal API Docs** (`/orgportal/admin/api-docs`).
